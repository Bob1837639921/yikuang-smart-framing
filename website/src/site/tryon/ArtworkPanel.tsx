...
type ArtworkPanelProps = {
  ...
  repairError?: string;
  uploadStatus: "idle" | "processing" | "error";
  uploadError?: string;
  onArtworkChange: (file: File) => void | Promise<void>;
  ...
  return (
    <aside className="try-panel try-artwork-panel" aria-label="作品设置">
      ...
      <label className={`try-upload${uploadStatus === "processing" ? " is-processing" : ""}`} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); acceptFile(event.dataTransfer.files[0]); }}>
        ...
        <span className="try-upload-overlay"><strong>{uploadStatus === "processing" ? "正在准备作品…" : "更换作品"}</strong><small>JPG、PNG、WebP · 最大 40 MB</small></span>
      </label>
      {uploadStatus === "error" && <p className="try-upload-error" role="alert">{uploadError}</p>}
      ...
      <section className="try-repair-panel" aria-label="作品去皱整理">
        ...
        >
          ...
          <div className="try-repair-after"><img src={artworkUrl} alt="整理后作品" style={{ width: compare <= 0 ? "100%" : `${10000 / compare}%` }} /></div>
