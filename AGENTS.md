...
- The legacy web prototype under `src/` is no longer maintained. Product UI and preview changes must target `miniprogram/` unless the user explicitly requests work on the web prototype or protected mobile runtime.
...
- Admin material creation must be explicit rather than a single generic upload: collect only the front texture and side texture, plus manually entered width, depth, visible side width, inner lip, bevel, material type, and profile type. Miter-corner and carved-detail uploads are not part of the current configuration flow. A ruler in the reference photo is for the administrator's measurement; it is not an automatic AI measurement step. Frame color is derived from the uploaded textures, so manual tone/edge color fields are not part of the configuration flow.
